...
help:
	...
	@echo "  make logs     实时日志（uvicorn + etl 交错输出）"
	@echo "  make status   容器状态 + 端口健康检查"
...
status:
	...
	@$(COMPOSE) ps
	@echo ""
	@echo "=== 健康检查 (3116/api/health) ==="
	@curl -s http://localhost:3116/api/health || echo "unreachable"
	@echo ""
	@echo "=== 页面 (3116) ==="
...
backup:
	@echo "备份 option-scanner-data 卷..."
	@mkdir -p ./backups
	docker run --rm -v option-scanner-data:/data -v "$$(pwd)":/backup alpine \
